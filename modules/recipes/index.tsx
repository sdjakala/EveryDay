...
export default function RecipesModule() {
  ...
  function handleCreateRecipe() {
    ...
    const recipe: Recipe = {
      ...
      planned: false,
      stepTimers: {},
      tags: tags.length > 0 ? tags : undefined, // ADD THIS LINE
    };
  ...
  function handleAddTag(recipeId: string, tag: string) {
    const recipe = recipes.find((r) => r.id === recipeId);
    if (!recipe) return;
    ...
    handleUpdateRecipe(recipeId, { tags: [...currentTags, trimmedTag] });
  }

  function handleRemoveTag(recipeId: string, tag: string) {
    const recipe = recipes.find((r) => r.id === recipeId);
  ...
  const displayedRecipes = recipes
    ...
    .filter((r) => (showPlanned ? r.planned : true))
  ...
  return (
    <div style={{ paddingTop: 10 }}>
      ...
      <div style={{ marginBottom: "1rem" }}>                
        ...
        <div style={{ 
          ...
          >
            ...
            <span style={{ fontSize: 13 }}>
              {showPlanned ? "Planned" : "Show planned"}
            </span>
          </button>
          ...
          {allTags.length > 0 && (
            <>
              <button
                ...
                className={`toggle-btn ${selectedTag === null ? "active" : ""}`}
                style={{ fontSize: 13 }}
              >
                All Tags
              </button>
              {allTags.map(tag => (
                <button
                  ...
                  className={`toggle-btn ${selectedTag === tag ? "active" : ""}`}
                  style={{ fontSize: 13 }}
                >
                  {tag}
                </button>
        ...
        {(searchQuery || selectedTag || showPlanned) && (
          <div style={{ 
            marginTop: "0.5rem", 
            fontSize: "0.85rem", 
            color: "var(--muted)" 
          ...
          >
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              <input
                ...
                value={createRecipeTags}
                onChange={(e) => setCreateRecipeTags(e.target.value)}
                style={{
                  width: "100%",
                  padding: 8,
                  marginBottom: 12,
      ...
      <div style={{ 
        ...
        {displayedRecipes.length === 0 ? (
          <div style={{ 
            ...
            No recipes found. {searchQuery || selectedTag ? "Try different filters." : "Create one to get started!"}
          </div>
        ) : (
                ...
                <div style={{ 
                  ...
                  {r.tags?.map(tag => (
                    ...
                    >
                      ...
                      {editingRecipeId === r.id && (
                        <button
                          onClick={() => handleRemoveTag(r.id, tag)}
                          style={{
                            background: "none",
                            border: "none",
                            color: "#ff6b6b",
                  ...
                  {/* Add Tag Button (when editing) */}
                  {editingRecipeId === r.id && (
                    <>
                      {editingTagsForRecipe === r.id ? (
                        <div style={{ display: "flex", gap: "4px" }}>
                          <input
                            type="text"
                            placeholder="New tag..."
                            ...
                            onKeyDown={(e) => {
                              if (e.key === "Enter") {
                                e.preventDefault();
                                handleAddTag(r.id, newTagInput);
                                setNewTagInput("");
                                setEditingTagsForRecipe(null);
                              }
                              if (e.key === "Escape") {
                                setNewTagInput("");
                                setEditingTagsForRecipe(null);
                        ...
                        <button
                          onClick={() => setEditingTagsForRecipe(r.id)}
                          style={{
                            padding: "2px 8px",
