...
export default function Dashboard() {
  ...
  const onDrop = (e: React.DragEvent<HTMLDivElement>, targetName: string) => {
    ...
    try {
      ...
      // persist order
      try {
        localStorage.setItem('moduleOrder', JSON.stringify(next.map(m => m.name)));
      } catch (err) {}
    } catch (err) {}
  };
  ...
  const onTouchStart = (e: React.TouchEvent<HTMLDivElement>, name: string) => {
    ...
    try {
      ...
      if (tg && tg.closest && tg.closest('button, a, input, select, textarea, label')) {
        return;
      }
    } catch (err) {}
  ...
  const onTouchMove = (e: React.TouchEvent<HTMLDivElement>) => {
    ...
    if (!el) return;
    const card = el.closest && (el.closest('.module-card') as HTMLElement | null);
  ...
  const onTouchEnd = (e: React.TouchEvent<HTMLDivElement>) => {
    ...
    if (srcName && tgtName && srcName !== tgtName) {
      ...
      if (srcIdx !== -1 && tgtIdx !== -1) {
        ...
        next.splice(tgtIdx, 0, moved);
        setModules(next);
        try { localStorage.setItem('moduleOrder', JSON.stringify(next.map(m => m.name))); } catch(err){}
      }
    }
    // cleanup
    setTouchStartPos(null);
    ...
    try {
      document.querySelectorAll('.module-card.dragging').forEach(el => el.classList.remove('dragging'));
    } catch (err) {}
  };
  ...
  return (
    <div className="container">
      <h1>Dashboard</h1>
      <p>Welcome, {user.name} — rank {user.rank}</p>

      {pinned ? (
        // if pinned, only show that module
        modules.filter(m=>m.name === pinned).map(mod => {
          if (!mod || !mod.enabled || user.rank < mod.minRank) return null;
          const Component = dynamic(() => import(`../modules/${mod.name}/index`).then(m => m.default), {ssr:false, loading: () => <div className="module-card">Loading {mod.name}...</div>});
          return (
            <div key={mod.name} className="module-card" style={{minHeight:'70vh'}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                <h3>{mod.name}</h3>
                <div style={{display:'flex',gap:8}}>
                  <button className="icon-btn" title="Unpin" onClick={() => { localStorage.removeItem('pinnedModule'); setPinned(null); }}><Icon name="unpin" /></button>
                </div>
              </div>
              <p>{mod.description}</p>
              <Component />
        ...
        modules.map(mod => {
          ...
          const Component = dynamic(() => import(`../modules/${mod.name}/index`).then(m => m.default), {
            ssr: false,
            loading: () => <div className="module-card">Loading {mod.name}...</div>
          });
          ...
          return (
            <div
              ...
              style={{cursor: 'grab', border: isOver ? '2px dashed var(--accent)' : undefined}}
            >
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                <h3>{mod.name}</h3>
                <div style={{display:'flex',gap:8}}>
                  <button className="icon-btn" title={`Pin ${mod.name}`} onClick={() => { try { localStorage.setItem('pinnedModule', mod.name); setPinned(mod.name); } catch(e){} }}><Icon name="pin" /></button>
                </div>
              </div>
              <p>{mod.description}</p>
              <Component />
